...
+ The ~hack-local-variables-hook~ hack is copied from [[https://emacs-lsp.github.io/lsp-python-ms][lsp notes]], so
  that lsp looks for the virtualenv executable in /.dir-locals.el/ for
  something like ~(python-mode . ((lsp-python-ms-python-executable . "/PATH/TO/VENV/PYTHONBINARY")))~
